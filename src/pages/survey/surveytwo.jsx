...
import { ReactComponent as CHtwo } from '../../Icon/이미지-설문2캐릭터.svg' 
...
export default function SurveyTwo(){
    ...
    const bannerItemstwo = [
      <TwonotBanner key="item3">10만원이 생긴다면?</TwonotBanner>,
      <TwoBanner key="item4">10만원이 생긴다면?</TwoBanner>,
    ];
    const btnContents = [
    { icon: '🐷' },
    { icon: '🛒' },
  ...
  useEffect(() => {
    ...
    const fetchQData = async () => {
      try {
        const response = await axiosInstance.get('/api/mbti/questions/2/options'); // baseURL 뒤에 붙을 경로만 작성
...
const PostQData = async () => {
    ...
    const body = {
        ...
        questionId: 2, // 현재 설문 문항 ID는 1로 고정
