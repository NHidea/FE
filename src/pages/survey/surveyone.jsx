...
import { ReactComponent as CHone } from '../../Icon/이미지－설문1캐릭터.svg' 
...
export default function SurveyOne(){
    ...
    const bannerItemstwo = [
      <TwonotBanner key="item3">가장 먼저 하는 행동은?</TwonotBanner>,
      <TwoBanner key="item4">가장 먼저 하는 행동은?</TwoBanner>,
    ];
...
// 아이콘만 유지
  ...
  useEffect(() => {
    ...
    const fetchQData = async () => {
      try {
        const response = await axiosInstance.get('/api/mbti/questions/1/options'); // baseURL 뒤에 붙을 경로만 작성
...
const PostQData = async () => {
    ...
    const body = {
        ...
        questionId: 1, // 현재 설문 문항 ID는 1로 고정
