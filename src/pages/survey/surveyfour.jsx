...
import { ReactComponent as CHfour } from '../../Icon/이미지-설문4캐릭터.svg'
...
export default function SurveyFour(){
    ...
    const bannerItemstwo = [
      <TwonotBanner key="item3">같이 할까, 말까?</TwonotBanner>,
      <TwoBanner key="item4">같이 할까, 말까?</TwoBanner>,
    ];
  ...
  useEffect(() => {
    ...
    const fetchQData = async () => {
      try {
        const response = await axiosInstance.get('/api/mbti/questions/4/options'); // baseURL 뒤에 붙을 경로만 작성
...
const PostQData = async () => {
    ...
    const body = {
        ...
        questionId: 4, // 현재 설문 문항 ID는 1로 고정
